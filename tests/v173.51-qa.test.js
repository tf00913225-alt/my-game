...
assert.match(battle,/window\.v148GetAutoTargetPriority\(list\)\.slice\(\)/);
assert.match(battle,/return \[list\[3\],list\[1\],list\[4\],list\[2\],list\[0\]\]/);
...
assert.ok(loader.includes('const V_ASSET_VERSION="173.66";'));
assert.doesNotMatch(ui,/equipment-progression\.js\?v=|createElement\(["']script["']\)/);
assert.ok(index.includes('<title>四象江湖傳 V173.66</title>'));
