...
test("Flood Beast keeps the full-frame default while the current cache publishes the fix",()=>{
    ...
    assert.match(index,/js\/00-main\.js\?v=173\.6/);
    assert.match(index,/js\/19-stage-v78-character-inventory-runtime\.js\?v=173\.6/);
    assert.match(index,/js\/20-anonymous-20\.js\?v=173\.6/);
});

test("Home cover exposes the current V173.6 build in its bottom-right safe area",()=>{
    assert.match(
        index,
