...
try{
    ...
    const result=await client.eval(`(()=>{const resources=performance.getEntriesByType("resource").filter(entry=>{try{return new URL(entry.name).origin===location.origin;}catch(_){return false;}});const resourcePath=entry=>{try{return new URL(entry.name).pathname.toLowerCase();}catch(_){return "";}};const mark=performance.getEntriesByName("four-symbols:auth-ui-interactive").at(-1);const navigation=performance.getEntriesByType("navigation")[0];const consent=JSON.parse(localStorage.getItem("four_symbols_privacy_consent")||"null");return {state:FourSymbolsStartupPolicy.getState(),uid:FourSymbolsStartupPolicy.getUid(),creation:getComputedStyle(document.getElementById("creationPage")).display,authVisible:document.getElementById("firebaseAuthOverlay")?.classList.contains("show")||false,labels:[...document.querySelectorAll("#firebaseSignedOutPanel button")].map(button=>button.textContent.trim()),consentVersion:consent&&consent.privacyPolicyVersion,authUiMs:mark?Math.round(mark.startTime*10)/10:null,firstPaintMs:Math.round((performance.getEntriesByName("first-paint")[0]?.startTime||0)*10)/10,requestCount:resources.length+1,jsRequestCount:resources.filter(entry=>resourcePath(entry).endsWith(".js")).length,cssRequestCount:resources.filter(entry=>resourcePath(entry).endsWith(".css")).length,imageRequestCount:resources.filter(entry=>[".png",".jpg",".jpeg",".webp",".svg"].some(extension=>resourcePath(entry).endsWith(extension))).length,transferredBytes:(navigation?.transferSize||0)+resources.reduce((sum,entry)=>sum+(entry.transferSize||0),0),featureResources:resources.map(entry=>entry.name).filter(name=>/app-shell|gameplay-core|feature-/.test(name))};})()`);
