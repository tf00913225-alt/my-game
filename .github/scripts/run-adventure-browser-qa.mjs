...
function measure(name,html){
 ...
 for(let i=0;i<rects.length;i++)for(let j=i+1;j<rects.length;j++){const area=overlap(rects[i].r,rects[j].r);if(area>4)overlaps.push([i,j,Math.round(area)]);}
...
function run(){
 ...
 document.getElementById("game-content").innerHTML=scenarios.home;void document.body.offsetHeight;
 const reduced=matchMedia("(prefers-reduced-motion: reduce)").matches;const reducedAnimation=getComputedStyle(document.getElementById("adventureHomeEntry")).animationName;
