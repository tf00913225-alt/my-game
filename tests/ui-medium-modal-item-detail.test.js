...
assert.match(detailCss,/#itemModal \.item-modal-box:has\(#itemEquipButton:disabled\)\{[\s\S]*?height:auto !important;/);
assert.match(detailCss,/#itemModal \.item-modal-box:has\(#itemEquipButton:disabled\) #itemModalStats\{[\s\S]*?max-height:180px !important;/);
assert.match(detailCss,/#itemModal \.item-modal-box:has\(#itemEquipButton:disabled\) \.item-modal-buttons\{[\s\S]*?margin-top:0 !important;/);
...
<div class="item-modal-box"><div id="itemModalIcon" class="item-modal-icon">物</div><div id="itemModalName" class="item-modal-name">測試物品</div><div id="itemModalStats" class="item-stat-list"></div><div class="item-modal-buttons"><button id="itemEquipButton" disabled>不可裝備</button><button>返回</button></div></div>
...
(function(){
 ...
 stats.innerHTML='<p>效果：回復最大 HP 的 10%</p><p>售價：20 金幣</p>';void box.offsetHeight;const potion=snap('potion');
 equip.disabled=false;equip.textContent='穿戴';stats.innerHTML=Array.from({length:45},(_,i)=>'<p>裝備屬性 '+i+'</p>').join('');void box.offsetHeight;const equipment=snap('equipment');
...
try{
    ...
    assert.ok(data.potion.box.height<data.equipment.box.height-60,"potion action dialog must collapse instead of reserving the equipment inspector height");
    ...
    assert.ok(data.potion.buttons.top-data.potion.stats.bottom<24,"potion actions must sit directly below the short information card without a large blank spacer");
    console.log("Headless Chrome: potion detail collapses cleanly while equipment retains the fixed Medium Modal inspector");
}finally{
