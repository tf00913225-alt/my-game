...
assert.match(css,/\.v-fixed-boss-footprint\{[\s\S]*position:absolute;[\s\S]*left:calc\(20% \+ 12px\);[\s\S]*right:calc\(20% \+ 12px\);[\s\S]*top:var\(--battle-enemy-safe-top\);[\s\S]*bottom:0;/);
