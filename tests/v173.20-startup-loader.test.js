...
assert.match(source,/const introLogoTargetMs=Math\.max\(5000,/);
assert.match(source,/const introCityTargetMs=5000/);
assert.match(source,/await delay\(introLogoTargetMs\);[\s\S]*?showCityScene[\s\S]*?await delay\(introCityTargetMs\)/);
assert.match(source,/CREATION_PORTRAITS/);
assert.match(source,/await prepareCreationRender\(\)/);
