...
test("V155 no longer owns a final-roster patch",()=>{
    const context=load();
    assert.equal(context.v155PatchFinalAbyssRoster,undefined);
    assert.doesNotMatch(source,/FINAL_BOSS_RULES|FINAL_ELITE_RULES|v155FinalAbyss/);
    assert.match(source,/v174TrueRealmFinal/);
    assert.doesNotMatch(source,/monster\.name!==\"北帝天尊\"|monster\.name!==\"東帝天尊\"|monster\.name!==\"天帝天尊\"/);
...
test("Extreme Emperor uses only Yuan Zu Blessing with independent cleanse and instant recovery",()=>{
    ...
    const extreme={name:"極帝天尊",v141Abyss:true,v141SupportSkillIds:["yuanZuBlessing"],alive:true,hp:300,maxHP:500,sp:200,maxSP:500,evasion:50,agility:50,activeBuffs:[],statusEffects:[{type:"burn"}]};
...
test("North Emperor cannot revive without revive in its carried support list",()=>{
    ...
    assert.equal(context.v155ResolveNorthSupport(0,true),false);
...
test("wind elite uses Dodge, never Stealth",()=>{
    let finishes=0;
    const elite={name:"天兵天將",element:"wind",v141Abyss:true,v141SupportSkillIds:["dodgeSkill"],alive:true,hp:100,maxHP:100,sp:100,maxSP:100,evasion:20,skillChance:1,activeBuffs:[]};
    const context=load({
