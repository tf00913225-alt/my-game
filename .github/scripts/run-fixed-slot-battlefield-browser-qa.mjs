...
.battle-monster,.battle-player{position:relative;border:2px solid #f33;box-shadow:0 0 8px #f33}.v174-battle-art{background-image:linear-gradient(#7db,#246);background-repeat:no-repeat}
...
<style>${geometryCss}</style><style>${bossCss}</style></head><body>
...
<section class="battle-info-region"><div class="battle-info-header-row"><button id="battleInfoToggle" class="battle-info-toggle" type="button" aria-expanded="false"><span>⌃</span></button><div id="battleTurnIndicator">第 1 回合</div></div><div id="battleInfo" class="battle-info">戰鬥資訊</div></section>
...
(function(){
 ...
 function enemyCard(index){var c=document.createElement('div');c.id='battleMonster'+index;c.className='battle-monster '+(index===4?'abyss-unit':'');c.dataset.rank=window.monsters[index].rank;c.innerHTML='<div class="card-status-overlay burn-overlay show"></div><div class="v174-battle-art"></div><div class="battle-monster-name">怪物'+index+'</div><div class="monster-status-badges"></div><div class="monster-hp"><div class="monster-hp-inner"></div><div class="monster-bar-text">1000</div></div><div class="monster-sp"><div class="monster-sp-inner"></div><div class="monster-bar-text">100</div></div>';return c;}
 function allyCard(index){var c=document.createElement('div');c.id='battlePlayerCard'+index;c.className='battle-player';c.innerHTML='<div class="v174-battle-art"></div><div class="battle-player-id">角色'+index+'</div><div class="monster-status-badges"></div><div class="hp-bar"><div class="hp-bar-inner"></div><div class="hp-bar-text">1000</div></div><div class="sp-bar"><div class="sp-bar-inner"></div><div class="sp-bar-text">100</div></div>';return c;}
 ...
 var drawer=document.querySelector('.battle-info-region');drawer.style.transition='none';var collapsedDrawer={region:rect('.battle-info-region'),toggle:rect('#battleInfoToggle'),info:rect('#battleInfo')};drawer.classList.add('is-expanded');var expandedDrawer={region:rect('.battle-info-region'),toggle:rect('#battleInfoToggle'),info:rect('#battleInfo')};drawer.classList.remove('is-expanded');
 addCards(5,6,false);
...
function runViewport(chrome,width,height){
    ...
    assert.ok(data.collapsedDrawer.toggle.right>=width-5&&data.collapsedDrawer.toggle.left>=width-70,"collapsed drawer handle stays in the bottom-right corner");
    ...
    assert.equal(data.boss.border,"0px","Boss root must not expose a legacy element frame");assert.equal(data.boss.boxShadow,"none","Boss root must not expose a legacy card glow");assert.equal(data.boss.hp.display,"block");assert.equal(data.boss.hp.visibility,"visible");assert.equal(data.boss.hp.height,10);assert.equal(data.boss.hp.position,"absolute");assert.equal(data.boss.hp.text,"1000");
    data.boss.reinforcements.concat(data.boss.objects).forEach(function(side){assert.ok(side.right<=data.boss.footprint.left+.5||side.left>=data.boss.footprint.right-.5,"Boss side Unit must not invade central footprint");});
...
try{
    ...
    const evidence={suite:"fixed-slot-battlefield-rendering-v2",passed:true,viewports:results.map(item=>item.viewport),checks:{threeStructuralRegions:true,centerControlsIsolated:true,centerBackgroundRemoved:true,bottomBattleInfoDrawer:true,drawerCollapsedAndExpanded:true,bottomRightDrawerHandle:true,largeElementBox:true,enemyCounts:[1,3,5,6,8,10],allyCounts:[1,2,3,4,5,6],equalEnemyCardGeometry:true,equalAllyCardGeometry:true,equalEnemyAllyResourceBarHeight:true,sixAllyNoOverlap:true,enlargedUnitCards:true,splitAllyFormation:true,deathDoesNotCompress:true,bossSingleEntity:true,bossSixSlotFootprint:true,bossHpVisible:true,bossRootCardFrameAbsent:true,bossSideUnits:["ENEMY_B1","ENEMY_B5","ENEMY_F1","ENEMY_F5"],bossSideUnitsDoNotOverlap:true,bossAndObjectReticles:true,dynamicCardlessImmediate:true,retiredFullCardStatusOverlayAbsent:true,vfxShapes:["single","tri","row","column","all"],vfxCenterUsesGeometry:true,vfxScaleStable:true,artworkKinds:["player","regular","elite","boss","abyss"],artworkNoBattlefieldClip:true,popupKinds:["damage","critical","heal","miss"],popupUsesSlotAnchor:true,legacyRuntimeGeometryNeutralized:true,noPageScroll:true},results};
