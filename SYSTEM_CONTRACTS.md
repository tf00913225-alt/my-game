...
- 已讀狀態只存 per-UID `localStorage` sidecar：`release-update-last-seen-version`、`release-update-last-seen-notice` 與提醒冷卻 `release-update-last-reminder`；不寫入 Cloud Save、UID、金幣、背包、裝備或任何權威遊戲資料。
