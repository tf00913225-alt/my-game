...
5. 這個專案目前**沒有 CI**；V137 起有一組針對高風險回歸的 Node 測試
   ...
   `tests/v146-system-polish.test.js`。驗證至少要包含：
...
## 目前狀態（截至 2026-08-28，V146 戰鬥／深淵／手機介面定案版）
...
改變就會改變實際行為。V137 已把順序固定；V146 現在完整順序為
...
- [ ] V146 暫時沿用 `assets/ui/home-shop.png`。使用者在另一個對話上傳的新去背商店
      icon 不在此工作樹、此對話亦無法取得；收到原始檔後再單獨替換並更新快取鍵。
