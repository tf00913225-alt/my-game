...
5. 這個專案目前**沒有 CI**；V137 起有一組針對高風險回歸的 Node 測試
   ...
   `tests/v139-economy-rested-exp.test.js`。驗證至少要包含：
...
## 目前狀態（截至 2026-08-27，V139 經濟／養成／休息經驗版）
...
改變就會改變實際行為。V137 已把順序固定；V139 現在完整順序為
`js/25` → `js/27` → `js/28` → `js/29` → `js/30` → `js/31` → `js/32`。
...
- [ ] 目前仍沒有 CI；V137 的 `tests/v137-regressions.test.js` 有 9 項高風險
      ...
      並保留可選的 `tests/v138-browser-smoke.js`。目前執行環境缺少可用的瀏覽器控制介面，
