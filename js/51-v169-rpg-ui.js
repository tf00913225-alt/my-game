...
(function installV169RpgUi(){
    ...
    function normalizeSlotText(slot,fallback){
        const text=String(slot&&slot.textContent||"").replace(/\s+/g," ").trim();
        return text||fallback;
    }

    function getCharacterSlotNodes(selected){
        const owner=selected&&selected.parentElement;
        if(!owner){ return []; }
        return Array.from(owner.children).filter(node=>{
            if(!(node instanceof HTMLElement)){ return false; }
            if(node.classList.contains("v169-character-hero-switcher")){ return false; }
            const onclick=node.getAttribute("onclick")||"";
            const text=normalizeSlotText(node,"");
            return node.classList.contains("character-showcase-choice") ||
                onclick.includes("openCharacterCreation") ||
                /Lv\.10解鎖|前兩名皆\s*Lv\.50/.test(text);
        }).slice(0,3);
    }

    function ensureHeroSwitcher(modal){
        const box=modal.querySelector(".home-feature-modal-box.wide");
        if(!box){ return null; }
        let switcher=box.querySelector(":scope > .v169-character-hero-switcher");
        if(switcher){ return switcher; }

        switcher=document.createElement("div");
        switcher.className="v169-character-hero-switcher";
        switcher.style.cssText="position:absolute;z-index:7;top:8px;right:58px;display:grid;grid-template-columns:30px 46px 30px;gap:4px;align-items:center;height:38px;";

        const previous=document.createElement("button");
        previous.type="button";
        previous.className="v169-character-slot-step";
        previous.dataset.direction="-1";
        previous.textContent="‹";
        previous.style.cssText="width:30px;min-width:30px;height:34px;min-height:34px;padding:0;font-size:20px;line-height:1;";

        const counter=document.createElement("span");
        counter.className="v169-character-slot-counter";
        counter.style.cssText="display:flex;align-items:center;justify-content:center;min-width:46px;height:34px;border:1px solid rgba(200,154,72,.26);border-radius:8px;color:#efd18a;background:rgba(13,10,7,.72);font-size:12px;font-weight:900;font-variant-numeric:tabular-nums;";

        const next=document.createElement("button");
        next.type="button";
        next.className="v169-character-slot-step";
        next.dataset.direction="1";
        next.textContent="›";
        next.style.cssText="width:30px;min-width:30px;height:34px;min-height:34px;padding:0;font-size:20px;line-height:1;";

        switcher.append(previous,counter,next);
        box.appendChild(switcher);
        return switcher;
    }

    function activateCharacterSlot(modal,selected,targetIndex){
        const slots=getCharacterSlotNodes(selected);
        ...
        const reason=normalizeSlotText(target,"此角色欄位尚未解鎖");
        if(typeof window.rpgAlert==="function"){
            void window.rpgAlert(reason,{title:"角色欄位",confirmText:"知道了"});
        }
    }

    function syncHeroSwitcher(modal,selected,index){
        const slots=getCharacterSlotNodes(selected);
        if(slots.length===0){ return; }
        slots.forEach((slot,slotIndex)=>{
            slot.dataset.v169CharacterSlot=String(slotIndex);
            if(slot===selected){
        ...
        const switcher=ensureHeroSwitcher(modal);
        if(!switcher){ return; }
        const previous=switcher.querySelector('[data-direction="-1"]');
        const next=switcher.querySelector('[data-direction="1"]');
        const counter=switcher.querySelector(".v169-character-slot-counter");
        const activeIndex=Math.max(0,slots.indexOf(selected)>=0?slots.indexOf(selected):index);
        if(counter){ counter.textContent=(activeIndex+1)+"/"+slots.length; }
        if(previous){
            previous.disabled=activeIndex<=0;
            previous.setAttribute("aria-label",activeIndex>0?"切換至"+normalizeSlotText(slots[activeIndex-1],"上一個角色"):"已是第一個角色");
            previous.onclick=()=>activateCharacterSlot(modal,selected,activeIndex-1);
        }
        if(next){
            next.disabled=activeIndex>=slots.length-1;
            next.setAttribute("aria-label",activeIndex<slots.length-1?"切換至"+normalizeSlotText(slots[activeIndex+1],"下一個角色"):"已是最後一個角色");
            next.onclick=()=>activateCharacterSlot(modal,selected,activeIndex+1);
        }
    }
    ...
    function syncCharacterHeroPresentation(){
        ...
        syncHeroSwitcher(modal,selected,index);
    }
