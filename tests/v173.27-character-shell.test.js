...
test("the development release advances to V173.27",()=>{
    assert.match(loader,/const V_ASSET_VERSION="173\.27"/);
    assert.match(index,/<title>四象江湖傳 V173\.27<\/title>/);
    assert.match(index,/aria-label="目前版本 V173\.27"/);
    assert.match(index,/>V173\.27<\/div>/);
});

console.log("\n"+passed+" V173.27 character-shell regression tests passed.");
