...
// 4. Main-city roster/HUD is first-screen UI. Move its renderer/style owner out of lazy gameplay-core.
{
  ...
  const eagerAnchor="    apply();\n\n})();";
  eager=replaceOnce(eager,eagerAnchor,"    apply();\n"+addition+"\n})();","eager main-city roster owner");
...
for(const path of [
  ...
  let s=read(path);
  s=s.replace(/mechanismCardRule,\/aspect-ratio:9 \\/ 16\//g,"mechanismCardRule,/aspect-ratio:4 \\/ 3/");
  write(path,s);
}
