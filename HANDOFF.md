...
5. 這個專案目前**沒有 CI**；V137 起有一組針對高風險回歸的 Node 測試
   `tests/v137-regressions.test.js`。驗證至少要包含：
...
## 目前狀態（截至 2026-08-27，V137 Claude 接手稽核修復版）
...
- [ ] 目前仍沒有 CI；V137 已新增 `tests/v137-regressions.test.js`（9 項高風險
      回歸），但尚未涵蓋完整戰鬥／副本 UI 流程。之後修改 loader、經驗曲線、
      背包交易、自動戰鬥或多人角色邏輯時，必須同步擴充並執行這組測試。
