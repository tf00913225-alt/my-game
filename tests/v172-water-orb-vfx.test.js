...
test("Flood Beast keeps the full-frame default while the current cache publishes the fix",()=>{
    ...
    assert.match(index,/js\/00-main\.js\?v=173\.10/);
    assert.match(index,/js\/19-stage-v78-character-inventory-runtime\.js\?v=173\.10/);
    assert.match(index,/js\/20-anonymous-20\.js\?v=173\.10/);
});

test("Home cover exposes the current V173.10 build in its bottom-right safe area",()=>{
    assert.match(
        index,
        /id="homeVersionBadge"[\s\S]*?class="home-version-badge"[\s\S]*?aria-label="目前版本 V173\.10"[\s\S]*?>V173\.10<\/div>/
    );
    assert.match(
        index,
