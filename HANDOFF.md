...
> `tests/v173.18-final-request.test.js` 共同代表「完整載入所有補丁後」的 V173.18
> 最終狀態；下方 V140～V173.17 各段落與同名測試只保留歷史版本紀錄，不能再拿來判定
> 目前數值。

### 目前最終值

- 正式 `main` 目前是 V173.17 merge commit
  `7b2239be41ebedd0fbf07a85ac4d0a577455465c`；V173.18 遊戲內容位於 `dev` code commit
  `8572adbd8a38311eede9418836f5a4c6596b3fbf`，依使用者順序尚未合併到 `main`。
...
- `tests/v170-final-spec-integration.test.js` 會在同一個 `vm.Context` 真正依上述順序
  執行 49 支 JavaScript，再檢查最終資料與行為；V173.18 另鎖定符咒格線、深淵戰後回程
  與下游不得再覆蓋四元素 owner。兩套測試必須一起通過。
...
| 水 | 冰霜箭雨 | 30／+6 | 75 | 敵方全體 | 20／1／5 | 洪水猛獸 |
...
| 風 | 風起雲湧 | 36／+7 | 75 | 敵方全體 | 30／1／5 | 風哮電擊 |
...
| 土 | 飛沙瞬擊 | 32／+6 | 55 | 敵方全體 | 15／1／5 | 滾石術 |
...
- 一般異常的等級差倍率為 `clamp(1 + 等級差×0.05, 0.5, 1.5)`；物理公式為
  `基礎機率×倍率 + 物攻×0.2 - 精神×0.3 - 額外抗性`，法術把物攻項改為智力×0.3，
...
- 火：火焰斬、會心一擊、火爆亂擊在目標死亡或任一爆擊時免費追擊一次；霸龍裂天斬最多
  ...
  `[5,7,9,11,13]%`，本次燃燒目標少於3人時下一回合傷害 +30%。怒火3回合的爆擊率為
...
- 水：水刀斬／冰霜拳／冰旋一閃／冰封重擊／水球術／洪水猛獸／冰霜箭雨的凍傷基礎機率
  ...
  機率、5回合、純控制。治療術 Lv1 為我方同排3人各 550 HP／65 SP、每級各+30並解除
  全部負面，施法者不吃自己的 SP 回復；復活術恢復 `[20,40,60,80,100]%` HP。水元素EX
  為水傷+5%、回復+10%，回合開始前30%解除自身全部負面。
- 風：暴風拳降低敏捷 `[30,40,50,60,70]%` 1回合；暴風亂擊、風旋十字斬、風焰術、
  風哮電擊依技能資料降低傷害；暈眩猛擊5回合與風起雲湧1回合的 MISS 增幅均為
  `[30,45,50,55,65]%`。閃躲術為同排3人+75%閃躲3回合；隱身3回合；氣定神閒為全隊
  異常抗性+65%、命中+50%共3回合；風元素EX永久閃躲+35%。
- 土：土石斬／落石術／滾石術／飛沙瞬擊依技能資料降低防禦；石盾拳與石破天驚給自身
  ...
  防禦+35%共4回合；結界擋5次直接傷害、最多5回合且 DOT 穿透；土元素EX永久防禦+35%。
...
- V140～V173.17 的舊技能斷言只代表歷史快照；不得再為了讓舊數值斷言通過而改回正式值。
  ...
  權威目標解析與治療結算是 `js/42`。
- `skillDatabase.yuanXiangGuangMing` 的歷史欄位仍由 V155 正式 resolver 隔離；本輪沒有改動
  極帝天尊技能。
...
### 2026-09-01 — V173.18：符咒格線、深淵戰後回圖與四元素技能最終定案（dev）
