...
test("V141 assets remain ordered before later patches with cache version 151",()=>{
    const paths=[
    ...
    assert.match(loaderSource,/const V_ASSET_VERSION="151"/);
    assert.match(indexSource,/js\/00-main\.js\?v=141/);
    assert.match(indexSource,/js\/20-anonymous-20\.js\?v=151/);
});
