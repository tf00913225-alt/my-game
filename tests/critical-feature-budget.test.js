...
assert.ok(fs.statSync(manifest.critical.scripts[0]).size<=45_000,"boot JavaScript budget exceeded");
assert.ok(fs.statSync(manifest.critical.styles[0]).size<=250_000,"boot CSS source budget exceeded");
assert.ok(localCriticalBytes<=650_000,"signed-out local critical source budget exceeded: "+localCriticalBytes);
