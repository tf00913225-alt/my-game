...
#bossMechanismSlot{position:absolute}.battle-middle{min-height:0}.v-fixed-action-zone{height:66px}.battle-info{min-height:0;overflow-y:auto}
...
<section class="battle-enemy-region"><div class="battle-title">戰鬥</div><div id="battleMonsterArea" class="battle-monsters v131-formation"></div><div id="bossMechanismSlot"></div></section>
<section class="battle-center-region"><div id="turnTargetRow" class="turn-target-row">第 1 回合</div><div class="battle-middle"><div id="battleActionRegion" class="v-fixed-action-zone">操作區</div></div><div class="battle-info-header-row"><div id="battleTurnIndicator">第 1 回合</div></div><div id="battleInfo" class="battle-info">戰鬥資訊</div></section>
...
(function(){
 ...
 function lower(){return {enemy:rect('.battle-enemy-region'),center:rect('.battle-center-region'),allyRegion:rect('.battle-ally-region'),ally:rect('#battlePlayerRow'),middle:rect('.battle-middle'),action:rect('#battleActionRegion'),info:rect('#battleInfo')};}
 ...
 var allyScenarios={};
 [1,2,3].forEach(function(count){addCards(5,count,false);allyScenarios[count]={lower:lower(),unitRects:unitRects('.v-fixed-ally-slot'),slots:document.querySelectorAll('.v-fixed-ally-slot').length,assigned:Object.assign({},owner.ensureAllyFormation(window.__allyIndexes).characterIndexToSlot)};});
...
function runViewport(chrome,width,height){
    ...
    assert.ok(baseline.info.top>=baseline.center.top-.5&&baseline.info.bottom<=baseline.center.bottom+.5,"battle info stays inside the center region");
