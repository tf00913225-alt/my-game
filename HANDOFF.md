...
> `tests/v173.18-final-request.test.js` 共同代表「完整載入所有補丁後」的 V173.24
> 最終狀態；下方 V140～V173.17 各段落與同名測試只保留歷史版本紀錄，不能再拿來判定
> 目前數值。

### 目前最終值

- 正式 `main` 目前是 V173.27（PR #35，merge SHA
  `024b2c7c34c14db7b4ee2ba4be3b7addcabac414`）；四元素法師套裝 icon、玩家／怪物閃躲與
  一般異常命中公式，以及 V173.25～V173.27 的手機 UI、角色頁共用黑色尾區根因修正，
  均已依 `dev → PR → main` 標準流程發布。V173.23 的風系施放與持續狀態 Sprite VFX、
  V173.22 的狂風術與四元素攻擊套裝 icon 均繼續保留。
...
- `tests/v170-final-spec-integration.test.js` 會在同一個 `vm.Context` 真正依上述順序
  執行 50 支 JavaScript，再檢查最終資料與行為；V173.24 另鎖定同名狀態先判定、正式狀態名、
...
- 玩家基礎閃躲為有效敏捷×0.6%；普通怪物預設基礎閃躲為
  ...
  `(1 - 最終閃躲率)` 並限制 1%～99%。風行75%、風元素EX35%、元祖賜福30%等額外來源
  維持獨立乘算，最終閃躲上限85%。
...
## 目前狀態（截至 2026-09-02，V173.30 dev）
...
- V173.30（本機 `dev`，尚未 push）在 V173.29 Lobby 基礎上完成第二輪精修：角色／商店主卡
...
### 2026-09-02 — V173.30：新版主城第二輪視覺層級精修（本機 dev，尚未 push）
...
- 完整本機 Repository checks 全數通過：JavaScript 語法 `166/166`、Node suites `52/52`、
  ...
  Cloud Browser 安全政策阻擋本機 data URL，因此 V173.30 未發布候選只能完成 CSS 幾何／事件
  合約驗收，最終像素級手機畫面仍待 push 後實機確認。
