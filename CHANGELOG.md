...
本版本收斂深淵與頭目戰的戰鬥畫面後段處理，減少重複流程造成的顯示不穩定；不調整角色／秘寶技能、Boss 數值、狀態回合、傷害公式、機率、EXP、金幣或掉落。

### 戰鬥穩定性
- 深淵與頭目戰的戰鬥畫面後段處理改由單一正式流程負責，降低重複執行造成的畫面呈現不穩定。
- 持續狀態、立繪顯示與傷害結算仍沿用既有規則；本次不新增玩法或改動戰鬥數值。

### 對應 Requirement Batch
- `2026-09-21-v17370-main-release`
...
本版本聚焦商店補品手機排版與主城隊伍秘寶摘要可讀性；不調整補品回復比例、價格、角色／秘寶技能數值、戰鬥公式、EXP（經驗值）、掉落或獎勵。

### 商店／補品
- 六種 HP／SP 補品維持 2×3 單頁排列，重新分配圖示、名稱、回復效果、數量、價格與購買按鈕空間，避免手機版文字與操作區互相重疊。
- 購買數量輸入區縮至僅容納 1～999 所需寬度，並把騰出的空間留給價格欄；購買上限與交易邏輯不變。
- 補品卡新增實際版面重疊回歸檢查，避免後層樣式再次把數量框撐回過大尺寸。

### 主城／隊伍秘寶
- 隊伍秘寶摘要移除次要觸發說明，只保留秘寶名稱、等級與更換操作。
- 壓縮秘寶 HUD 與更換按鈕的垂直高度，降低與底部固定導航互相遮擋的情況。
