...
> `tests/v173.18-final-request.test.js` 共同代表「完整載入所有補丁後」的 V173.32
...
- 正式 `main` 目前仍是 V173.24（SHA `5256564115b613c0ea4bab1d97506a5734aff8ee`）；
  V173.25～V173.32 目前只存在 `dev`。本輪依使用者明確要求只提交／推送 `dev`，不合併 main。
...
- `tests/v170-final-spec-integration.test.js` 會在同一個 `vm.Context` 真正依上述順序
  執行 50 支 JavaScript，再檢查最終資料與行為；V173.32 另鎖定同名狀態先判定、正式狀態名、
...
- 極帝天尊最終三招由 V155 resolver 結算：元相光明全體 +150 HP／+55 SP（成本35）；
  元光護體全體 100 護盾 2 回合（成本40）；元祖賜福逐目標獨立進行 25% 淨化判定，並使
  每個新套用祝福的目標閃避增加35%、持續2回合（成本45）；閃躲採正式乘算與85%上限，重施
  同名元祖賜福直接 MISS，不刷新。
...
## 目前狀態（截至 2026-09-02，V173.32 dev）
