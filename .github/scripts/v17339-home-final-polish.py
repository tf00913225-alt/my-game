...
def apply():
    ...
    # Existing utility-button owner only. 68px side padding preserves about a 60px
    # central gate corridor on the real 396px utility strip after the requested growth.
    patch_property("css/00-main.css",".home-utility-actions","grid-template-columns","repeat(2,86px)","repeat(2,100px)")
    patch_property("css/00-main.css",".home-utility-actions","padding","0 82px","0 68px")
    ...
    # Existing version-badge owner only.
    ...
    # Keep the existing main-city regression aligned with the new owner values.
    ...
    replacements=[
        ...
        (r'padding:0 82px/',r'padding:0 68px/'),
        ...
        ('assert.equal(88-82,6);','assert.equal(82-68,14);'),
