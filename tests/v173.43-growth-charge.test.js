...
function harness(options={}){
    ...
    return {context,alerts,setNow:value=>{now=Number(value);},getNow:()=>now,state:()=>JSON.parse(store.get(KEY)||"{}")};
}
...
test("1. Lv1–19 has no natural charge",()=>{
    ...
    assert.equal(h.context.v173PreviewExpPoolCharge().gain,0);assert.equal(h.context.v173GetExpPoolChargeState().unlocked,false);
});
test("2. Lv20 unlocks and shows the unlock notice only once",()=>{
    const h=harness({levels:[20]});assert.equal(h.context.v173GetExpPoolChargeState().unlocked,true);
    h.context.v173EnsureExpPoolChargeUnlocked(h.getNow(),true);h.context.v173EnsureExpPoolChargeUnlocked(h.getNow(),true);
    assert.equal(h.alerts.length,1);assert.match(h.alerts[0],/持續充能已解鎖/);
});
test("3. Lv1→20 total requirement is 37,950 EXP",()=>{assert.equal(harness().context.v173GetBeginnerGrowthAudit().totalExpTo20,37950);});
test("4. newcomer estimate sits in the 20–30 minute target",()=>{
    const audit=harness().context.v173GetBeginnerGrowthAudit();assert.equal(audit.newcomerOneTimeExp,6000);
    const battles=Math.ceil(5850/617)+Math.ceil((audit.totalExpTo20-6000-5850)/3255);const minutes=battles+6;
...
test("16. representative charge rates are smooth and monotonic",()=>{
    const h=harness({levels:[99]});const rates=[20,30,40,49,50,60,70,80,90,95,99].map(l=>h.context.v173GetNaturalChargeLevelsPerDay(l));
...
test("17. Lv49→50 has no cliff",()=>{
    const h=harness({levels:[50]});assert.ok(h.context.v133GetExpNextForLevel(50)/h.context.v133GetExpNextForLevel(49)<1.10);
...
test("18. Lv50+ slows progressively rather than halving",()=>{
    const h=harness({levels:[99]});const rates=[50,60,70,80,90,99].map(l=>h.context.v173GetNaturalChargeLevelsPerDay(l));
...
test("27. Lv100 cannot consume pool EXP",()=>{assert.match(growth,/character&&\(Number\(character\.level\)\|\|1\)>=MAX_CHARACTER_LEVEL/);assert.match(growth,/已達 Lv\."\+MAX_CHARACTER_LEVEL\+" 滿等/);});
...
test("29. entering the page never clears EXP guidance",()=>{
    assert.match(flow,/if\(expPage&&tabContent\.contains\(expPage\)\)\{ guideExpPool\(attention\); \}/);
    assert.doesNotMatch(flow,/function guideExpPool\(attention\)\{[\s\S]{0,900}canLevel\s*=\s*false/);assert.doesNotMatch(flow,/exp.*read/i);
});
