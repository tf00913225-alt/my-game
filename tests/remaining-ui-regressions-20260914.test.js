...
assert.doesNotMatch(battle,/battle-monster\.v174-cardless-unit>\.battle-monster-name\{[\s\S]*top:0!important/,"V173.51 presentation layer must not own HUD geometry");
assert.match(fixedSlotCss,/\.v-fixed-enemy-slot \.battle-monster-name\{[\s\S]*top:0 !important;[\s\S]*z-index:24 !important;/,"fixed Slot CSS must own enemy-name HUD geometry");
assert.doesNotMatch(battle,/battle-monster\.v174-cardless-unit>\.v174-battle-art\{[\s\S]*inset:/,"V173.51 presentation layer must not own artwork bounds");
assert.match(fixedSlotCss,/\.v-fixed-battle-slot > \.battle-monster > \.v174-battle-art,[\s\S]*left:calc\(-1 \* var\(--battle-art-overhang-x\)\) !important;[\s\S]*top:calc\(-1 \* var\(--battle-art-overhang-top\)\) !important;[\s\S]*bottom:var\(--battle-art-hud-reserve\) !important;/,"fixed Slot CSS must own battle artwork bounds");
