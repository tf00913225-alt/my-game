...
const renderAdapter=read("js/battlefield-render-geometry-adapter.js");
...
assert.match(fixedSlotCss,/\.v-fixed-enemy-slot \.battle-monster-name\{[\s\S]*top:0 !important;[\s\S]*z-index:24 !important;/);
assert.match(fixedSlotCss,/\.v-fixed-battle-slot > \.battle-monster > \.v174-battle-art,[\s\S]*top:calc\(-1 \* var\(--battle-art-overhang-top\)\) !important;[\s\S]*bottom:var\(--battle-art-hud-reserve\) !important;/);
assert.match(renderAdapter,/neutralizeLegacyPresentationGeometry\(\)[\s\S]*style\.textContent="";[\s\S]*presentationSource="fixed-slot-battlefield-rendering-v2\.css"/);
assert.doesNotMatch(battle,/battle-monster\.v174-cardless-unit>\.battle-monster-name\{[\s\S]{0,500}?top:/,"V173.51 presentation layer must not reclaim monster-name geometry");
