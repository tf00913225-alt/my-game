...
為避免需求、立繪尺寸與程式施工溝通混淆，本專案固定使用以下稱呼：

- **大 Boss**：只指 **世界 Boss、個人 Boss**。
- **小 Boss**：指深淵等一般首領，例如 **五帝天尊**。
- **大／小只代表立繪／資產尺寸分類**；不得據此推論戰力、難度、稀有度、劇情地位、掉落或玩法階級。
- 當立繪尺寸或資產分類會影響施工時，不得只寫模糊的「Boss」，必須明確寫成「大 Boss」或「小 Boss」。
- 既有程式內部的 `boss` ID、Registry key、資料夾路徑或 legacy 名稱，不因本術語規則自動重命名；除非另有遷移任務，避免為文字一致而破壞 Runtime。
- 既有已核准深淵五帝素材仍依本文件的 grandfathered asset 規則保留，不因稱呼改為「小 Boss」就自動 resize 或重製。
