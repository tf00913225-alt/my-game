...
(function(){
 ...
 function rect(sel){var node=document.querySelector(sel),r=node.getBoundingClientRect();return {left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height};}
 ...
 function artEvidence(selector){var art=document.querySelector(selector+' .v174-battle-art'),s=getComputedStyle(art);return {backgroundSize:s.backgroundSize,overflow:s.overflow,clipChain:clipChain(art),rect:rect(selector)};}
 var scenarios={};
 [1,3,5,6,8,10].forEach(function(count){addCards(count,3,false);var snap=owner.getActiveEnemySnapshot(),assigned=Object.assign({},snap.monsterIndexToSlot),primary=assigned[0];var single=adapter.getVfxGeometry('monster',primary,'single'),tri=adapter.getVfxGeometry('monster',primary,'tri'),row=adapter.getVfxGeometry('monster',primary,'row'),column=adapter.getVfxGeometry('monster',primary,'column'),all=adapter.getVfxGeometry('monster',primary,'all');scenarios[count]={lower:lower(),assigned:assigned,single:single&&{width:single.baseWidth,height:single.baseHeight},tri:tri&&{width:tri.baseWidth,height:tri.baseHeight},row:row&&{width:row.baseWidth,height:row.baseHeight},column:column&&{width:column.baseWidth,height:column.baseHeight},all:all&&{width:all.baseWidth,height:all.baseHeight},slots:document.querySelectorAll('.v-fixed-enemy-slot').length};});
 var allyScenarios={};
 ...
 addCards(3,3,true);var beforeDeath={e1:rect('#battleMonster1'),e2:rect('#battleMonster2')};document.getElementById('battleMonster1').remove();var afterDeath={e2:rect('#battleMonster2')};var targetSlot=owner.getEnemySlotForMonster(owner.getActiveEnemySnapshot(),2);
 window.showDamagePopup(document.getElementById('battleMonster2'),'-777','hp',true);window.showMissEffect(false,2,'MISS');var damage=Array.from(document.querySelectorAll('.damage-popup.hp-popup')).pop(),miss=Array.from(document.querySelectorAll('.damage-popup.miss-popup')).pop();
...
function runViewport(chrome,width,height){
    ...
    for(const [kind,art] of Object.entries(data.artwork)){assert.equal(art.backgroundSize,"contain",`${kind} artwork must use contain`);assert.equal(art.overflow,"visible",`${kind} artwork overflow`);assert.equal(art.clipChain.some(entry=>/hidden|clip/.test(entry.overflow)||/hidden|clip/.test(entry.overflowX)||/hidden|clip/.test(entry.overflowY)),false,`${kind} artwork has clipping ancestor`);}
    assert.equal(data.popup.damage.slot,data.targetSlot);assert.equal(data.popup.miss.slot,data.targetSlot);assert.ok(data.popup.damage.left&&data.popup.damage.top);assert.ok(data.popup.miss.left&&data.popup.miss.top);assert.equal(data.stageOverflow,"visible");
...
try{
    ...
    const evidence={suite:"fixed-slot-battlefield-rendering-v2",passed:true,viewports:results.map(item=>item.viewport),checks:{enemyCounts:[1,3,5,6,8,10],allyCounts:[1,2,3],splitAllyFormation:true,deathDoesNotCompress:true,vfxShapes:["single","tri","row","column","all"],vfxScaleStable:true,artworkKinds:["player","regular","elite","boss","abyss"],artworkNoBattlefieldClip:true,popupUsesSlotAnchor:true,noPageScroll:true},results};
