...
本版本整合 9 月 23 日已完成驗證的戰鬥 Runtime、命中／閃躲／異常公式、硬控狀態、技能成長、秘寶 Runtime 與巡怪／戰鬥資訊介面收斂。

### 戰鬥操作與效能
- 移除多層戰鬥 UI Wrapper、全域 Observer／Polling 與不必要的熱路徑更新，降低手動選目標、查看狀態與開啟資訊抽屜時的卡頓。
- Battle Info Tab 固定於右下，不再拖曳；技能、物品與目標選擇期間回合／倒數列保持 100% 可見並位於選擇 UI 上層。
- 目標準星、MISS 浮字、回合／目標提示幾何與戰鬥結果閱讀層持續收斂，避免重複視覺 Owner 互相覆蓋。

### 命中、異常與硬控
- 一般命中、怪物閃躲與異常狀態機率統一由正式最終判定公式處理；Boss／精英／一般單位沿用各自正式硬控上限。
- 冰封與石化正式互斥，不覆蓋、不刷新另一個既有硬控；冰封／石化固定包覆層位於其他 Body Status 輪播之下。
- 凍傷維持 Soft Debuff：傷害、最終閃躲與最終異常狀態抗性各降低 25%，不再具有禁止施法效果。

### 技能與秘寶
- 31 個玩家直接傷害技能正式擴充至 Lv10；所有可升級技能每次升級固定消耗 1 技能點，Lv5／Lv10 使用同一正式突破成長曲線。
- 炎魂共鳴、焚血訣、水／風／土輔助技能與岩石壁壘的等級、SP、目標與效果由正式 Skill Data 統一控制；三人增益／治療 VFX 以玩家實際選擇目標為主要錨點。
- Team Relic Runtime 固定隨 gameplay-core 準備；已完成效果的秘寶使用單一 Trigger／Effect Engine，未完成效果的秘寶不再假裝可裝備或觸發。

### 巡怪與介面
- 巡怪底部導覽與副本／Gameplay 共用五鍵 Owner，順序固定為「角色／背包／秘寶／元素匣／返回」，移除右上角第二顆返回鈕。
- 巡怪角色直接顯示玩家正式性別／元素外觀，形象切換移至右上騰出的區域。
- 技能說明中的「個百分點」顯示統一改為 %，self 技能 VFX 固定錨定施放者本人。

### 對應 Requirement Batches
- `2026-09-23-battle-ui-hit-status-owner-convergence`
- `2026-09-23-freeze-petrify-exclusive-status-layer`
- `2026-09-23-skill-relic-runtime-owner-convergence`
- `2026-09-23-v17372-main-release`
...
- `2026-09-23-v17371-main-release`
