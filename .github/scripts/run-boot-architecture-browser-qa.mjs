...
try{
    ...
    const cityVisual=await client.eval(`(()=>{const mark=name=>performance.getEntriesByName(name).at(-1)?.startTime||0;const images=[...document.querySelectorAll("#homePage img,#bottomNav img")];const home=document.getElementById("homePage");const backgrounds=[home,...home.querySelectorAll(".home-bg-fixed-layer,.home-card-icon")].map(node=>getComputedStyle(node).backgroundImage).filter(value=>value&&value!=="none"&&/url\\(/.test(value));return {dataReady:mark("four-symbols:main-city-data-ready"),visualReady:mark("four-symbols:main-city-visual-ready"),interactive:mark("four-symbols:main-city-interactive"),loaderHidden:document.getElementById("startupLoader").hidden,rosterReady:document.getElementById("v146HomeRoster")?.dataset.ready||null,imagesDecoded:images.every(image=>image.complete&&image.naturalWidth>0),backgroundsReady:backgrounds.length>0&&backgrounds.every(value=>value&&value!=="none")};})()`);
    ...
    await waitFor(client,"performance.getEntriesByName('four-symbols:relic-visual-ready').length>0&&document.querySelectorAll('.team-relic-card-art img').length===20","cold relic visual ready",20000);
    const relicReady=await client.eval(`(()=>({runtime:performance.getEntriesByName("four-symbols:relic-runtime-ready").length,visual:performance.getEntriesByName("four-symbols:relic-visual-ready").length,cards:document.querySelectorAll(".team-relic-card-art img").length,decoded:[...document.querySelectorAll(".team-relic-card-art img")].every(image=>image.complete&&image.naturalWidth>0),loading:!!document.querySelector(".team-relic-loading")}))()`);
    assert.ok(relicReady.runtime>=1&&relicReady.visual>=1);assert.equal(relicReady.cards,20);assert.equal(relicReady.decoded,true);assert.equal(relicReady.loading,false);evidence.checks.relicFirstOpen=relicReady;
    await client.eval(`document.querySelector(".home-feature-close-btn")?.click()`);
    ...
    evidence.checks.mainCity412x915=await client.eval(`(()=>{const images=[...document.querySelectorAll("#homePage img,#bottomNav img")];return {viewport:[innerWidth,innerHeight],overflow:document.documentElement.scrollWidth>innerWidth,loaderHidden:document.getElementById("startupLoader").hidden,imagesDecoded:images.every(image=>image.complete&&image.naturalWidth>0),visualReady:performance.getEntriesByName("four-symbols:main-city-visual-ready").length>0};})()`);
