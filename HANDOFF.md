...
- 正式 `main` 目前是 V173.24；四元素法師套裝 icon、玩家／怪物閃躲與一般異常命中公式
  已依 `dev → PR → main` 標準流程發布。V173.23 的風系施放與持續狀態 Sprite VFX、
...
## 目前狀態（截至 2026-09-01，V173.25 dev）
...
- V173.25（僅 `dev`，待使用者實機驗收）修正裝備詳細視窗裁切、能力明細關閉鍵、能力頁黑色
  留白、兌換券完整預覽與二層裝備明細；凍傷技能封鎖改為純文字，技能名稱依各技能動畫總長
  顯示 2/3，戰鬥異常文字移到卡牌下方並完整顯示 1 秒。`main` 仍維持 V173.24。
...
- `css/00-main.css` 讓「穿戴／脫下」與「售出」共用完全相同的啟用漸層、字色；裝備大圖改成
  ...
  都由同一個彈窗高度管理。
- 背包放大鏡能力明細改成「外框 flex、能力格線單一捲動、關閉鍵固定」；能力值頁黑色方框深追
  後確認是 `css/22-stage-v78-character-inventory-core.css` 的 `padding-bottom:160px !important`
  在實際載入順序中覆蓋後載補丁所致，已直接從真正 owner 移除，不再保留第二個覆蓋來源。
