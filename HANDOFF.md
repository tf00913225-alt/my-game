...
5. 這個專案目前**沒有 CI**；V137 起有一組針對高風險回歸的 Node 測試
   ...
   `tests/v138-feature-requirements.test.js`。驗證至少要包含：
...
## 目前狀態（截至 2026-08-27，V138 副本／戰鬥節奏／獎勵介面版）
...
改變就會改變實際行為。V137 已把順序固定為 `js/25` → `js/27` → `js/28`
→ `js/29` → `js/30` → `js/31`。
...
- [ ] 目前仍沒有 CI；V137 的 `tests/v137-regressions.test.js` 有 9 項高風險
      ...
      並新增可選的 `tests/v138-browser-smoke.js`。目前執行環境缺少 Chromium，
      所以完整戰鬥／副本 UI 點擊流程仍未自動化跑過；之後修改 loader、經驗
