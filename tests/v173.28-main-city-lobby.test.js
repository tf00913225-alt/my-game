...
test("the compact HUD replaces the repeated giant city title",()=>{
    ...
    assert.match(baseCss,/\.home-city-hud\{[\s\S]*min-height:50px;[\s\S]*background:/);
});
...
test("DEV gold and EXP shortcuts remain direct but visually recessive",()=>{
    ...
    assert.match(baseCss,/\.home-test-button\{[\s\S]*height:20px;[\s\S]*opacity:.78/);
...
test("the roster renderer owns HUD data and preserves three vertical party cards",()=>{
    assert.match(rosterRuntime,/const partyIndexes=getExistingPartyIndexes\(\)/);
    assert.match(rosterRuntime,/homeHudCharacterName/);
    ...
    assert.match(rosterCss,/\.v146-home-roster\{[\s\S]*display:grid;[\s\S]*gap:3px/);
    assert.match(rosterCss,/\.v146-home-character\{[\s\S]*grid-template-columns:39px minmax\(0,1fr\)/);
...
test("development cache and visible version advance to V173.28",()=>{
    ...
    assert.match(index,/js\/00-main\.js\?v=173\.28/);
    assert.match(index,/js\/20-anonymous-20\.js\?v=173\.28/);
});

console.log("\n"+passed+" V173.28 main-city lobby tests passed.");
