...
try{
    ...
    const cityVisual=await client.eval(`(()=>{const mark=name=>performance.getEntriesByName(name).at(-1)?.startTime||0;const images=[...document.querySelectorAll("#homePage img,#bottomNav img")];const home=document.getElementById("homePage");const backgrounds=[home,...home.querySelectorAll(".home-bg-fixed-layer,.home-card-icon")].map(node=>getComputedStyle(node).backgroundImage);return {dataReady:mark("four-symbols:main-city-data-ready"),visualReady:mark("four-symbols:main-city-visual-ready"),interactive:mark("four-symbols:main-city-interactive"),loaderHidden:document.getElementById("startupLoader").hidden,rosterReady:document.getElementById("v146HomeRoster")?.dataset.ready||null,imagesDecoded:images.every(image=>image.complete&&image.naturalWidth>0),backgroundsReady:backgrounds.every(value=>value&&value!=="none")};})()`);
