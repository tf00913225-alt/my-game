...
<section class="battle-info-region"><div class="battle-info-header-row"><div id="battleTurnIndicator">第 1 回合</div></div><div id="battleInfo" class="battle-info">戰鬥資訊</div></section>
...
(function(){
 ...
 function lower(){return {enemy:rect('.battle-enemy-region'),center:rect('.battle-center-region'),allyRegion:rect('.battle-ally-region'),ally:rect('#battlePlayerRow'),infoRegion:rect('.battle-info-region'),middle:rect('.battle-middle'),action:rect('#battleActionRegion'),info:rect('#battleInfo'),elementBox:rect('.battle-element-box-button')};}
 ...
 var allyScenarios={};
 [1,2,3,4,5,6].forEach(function(count){addCards(5,count,false);allyScenarios[count]={lower:lower(),unitRects:unitRects('.v-fixed-ally-slot'),slots:document.querySelectorAll('.v-fixed-ally-slot').length,assigned:Object.assign({},owner.ensureAllyFormation(window.__allyIndexes).characterIndexToSlot)};});
...
function runViewport(chrome,width,height){
    ...
    assert.ok(baseline.allyRegion.bottom<=baseline.infoRegion.top+.5,"ally region must end before the bottom battle info region");
    ...
    assert.ok(baseline.info.top>=baseline.infoRegion.top-.5&&baseline.info.bottom<=baseline.infoRegion.bottom+.5,"battle info stays inside the bottom region");
...
try{
    ...
    const evidence={suite:"fixed-slot-battlefield-rendering-v2",passed:true,viewports:results.map(item=>item.viewport),checks:{fourStructuralRegions:true,centerControlsIsolated:true,bottomBattleInfo:true,largeElementBox:true,enemyCounts:[1,3,5,6,8,10],allyCounts:[1,2,3,4,5,6],equalEnemyCardGeometry:true,equalAllyCardGeometry:true,sixAllyNoOverlap:true,enlargedUnitCards:true,splitAllyFormation:true,deathDoesNotCompress:true,vfxShapes:["single","tri","row","column","all"],vfxCenterUsesGeometry:true,vfxScaleStable:true,artworkKinds:["player","regular","elite","boss","abyss"],artworkNoBattlefieldClip:true,popupKinds:["damage","critical","heal","miss"],popupUsesSlotAnchor:true,legacyRuntimeGeometryNeutralized:true,noPageScroll:true},results};
