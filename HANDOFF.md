...
5. 這個專案目前**沒有 CI**；V137 起有一組針對高風險回歸的 Node 測試
   ...
   `tests/v144-rules-and-abyss.test.js`。驗證至少要包含：
...
## 目前狀態（截至 2026-08-28，V144 商店／怪物技能與深淵第五關定案版）
...
改變就會改變實際行為。V137 已把順序固定；V144 現在完整順序為
`js/25` → `js/27` → `js/28` → `js/29` → `js/30` → `js/31` → `js/32` → `js/33`
→ `js/34` → `js/35` → `js/36` → `js/37` → `js/38` → `js/39` → `js/40`。
...
所以尚未完成真實手機視覺／點擊驗收；依既定流程只提交至 `dev`，合併 `main` 前應
