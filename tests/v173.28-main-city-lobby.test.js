...
test("development cache and visible version stay synchronized",()=>{
    ...
    assert.match(index,/aria-label="目前版本 V173\.60"[\s\S]*?>V173\.60<\/div>/);
    assert.match(index,/css\/00-main\.css\?v=173\.60/);
    assert.match(index,/css\/19-stage-v54-main-city-moderate-native-scale\.css\?v=173\.60/);
    assert.match(index,/js\/00-main\.js\?v=173\.60/);
    assert.match(index,/js\/16-stage-v54-main-city-runtime\.js\?v=173\.60/);
